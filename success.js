...
if(!order){
  ...
  document.querySelector("#invoicePayment").innerHTML=`<span>Subtotal: <strong>${money(order.subtotal)}</strong></span><span>Convenience Fee (3%): <strong>${money(order.platformFee || 0)}</strong></span><span>${order.handlingLabel || (order.orderType === "Delivery" ? "Delivery & Handling" : "Packing & Disposables")}: <strong>${money(order.handlingFee || 0)}</strong></span><span>Payment method: <strong>Razorpay</strong></span><span>Payment status: <strong>Verified & Captured</strong></span><span>Payment ID: <strong>${order.paymentId}</strong></span>`;
