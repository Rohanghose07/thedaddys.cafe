...
function indiaNowParts(){const f=new Intl.DateTimeFormat("en-US",{timeZone:"Asia/Kolkata",weekday:"long",hour:"2-digit",minute:"2-digit",hour12:false});const o={};for(const p of f.formatToParts(new Date()))if(p.type!=="literal")o[p.type]=p.value;return{day:String(o.weekday).toLowerCase(),minutes:Number(o.hour)*60+Number(o.minute)}}
function tm(t){const [h,m]=String(t).split(":").map(Number);return h*60+m}
async function getStoreAvailability(){
  const url=process.env.SUPABASE_URL,key=process.env.SUPABASE_SERVICE_ROLE_KEY;
  if(!url||!key)return {open:true};
  try{const r=await fetch(`${url}/rest/v1/store_settings?id=eq.1&select=ordering_enabled,business_hours`,{headers:{apikey:key,Authorization:`Bearer ${key}`}});if(!r.ok)return{open:true};const cfg=(await r.json())[0];if(!cfg)return{open:true};if(cfg.ordering_enabled===false)return{open:false,error:"The cafe is temporarily closed for online orders."};const p=indiaNowParts(),slots=cfg.business_hours?.[p.day]||[];return{open:slots.some(s=>p.minutes>=tm(s[0])&&p.minutes<tm(s[1])),error:"The cafe is currently closed. Please place your order during opening hours."}}catch(e){console.error("availability check failed",e);return{open:true}}
}
...
export default async function handler(req, res){
  ...
  try {
    ...
    if(!availability.open)return json(res,403,{error:availability.error});
