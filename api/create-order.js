...
export default async function handler(req, res){
  ...
  try {
    const {items, orderType, receipt} = req.body || {};
    ...
    const handlingFee = orderType === "Delivery" ? 30 : 15;
    ...
    const rp = await fetch("https://api.razorpay.com/v1/orders", {
      ...
      body:JSON.stringify({
        ...
        notes:{order_type:orderType, subtotal:String(subtotal), handling_fee:String(handlingFee)}
      })
    });
