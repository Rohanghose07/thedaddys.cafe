...
function totals(){
  ...
  const handlingFee = cart.length ? (orderType === "Delivery" ? 30 : 15) : 0;
  return {
...
function renderCart(){
  const {count,subtotal,handlingFee,total} = totals();
  $("#headerCartCount").textContent = count;
  $("#mobileCartCount").textContent = `${count} item${count===1?"":"s"}`;
  $("#mobileCartTotal").textContent = money(total);
  $("#cartSubtotal").textContent = money(subtotal);
  $("#cartFeeLabel").textContent = orderType === "Delivery" ? "Delivery & Handling" : "Packaging & Handling";
  $("#cartHandlingFee").textContent = money(handlingFee);
...
function buildOrder(){
  ...
  const {subtotal,handlingFee,total} = totals();
...
*Subtotal:* ₹${subtotal}
*${orderType === "Delivery" ? "Delivery & Handling" : "Packaging & Handling"}:* ₹${handlingFee}
...
Thank you.`;
  ...
  return {
    ...
    order: {
      ...
      subtotal,
      handlingFee,
      handlingLabel: orderType === "Delivery" ? "Delivery & Handling" : "Packaging & Handling",
      total,
...
$("#whatsappCheckoutBtn").addEventListener("click", ()=>{
  ...
  localStorage.setItem("tdcPendingOrder", JSON.stringify(result.order));
