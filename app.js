...
function renderMenu(){
  ...
  $("#menuGrid").innerHTML = filtered.length ? filtered.map(x => `
    <article class="menu-card">
      <div class="menu-card-top">
        <div>
          <h3>${x.name}</h3>
          <div class="menu-meta">${x.category}${x.subcategory ? " • "+x.subcategory : ""}</div>
        </div>
      </div>
    ...
    </article>`).join("") : `<div class="empty-cart">No dishes match your search.</div>`;
  $$(".add-btn").forEach(btn => btn.addEventListener("click", () => addItem(decodeURIComponent(btn.dataset.name))));
}
...
function buildOrder(){
  ...
  const pincode = $("#deliveryPincode") ? $("#deliveryPincode").value.trim() : "";
  const note = $("#orderNote").value.trim();
  if(!name) return {error:"Please enter your name."};
  if(orderType==="Delivery" && !address) return {error:"Please enter your full delivery address."};
  if(orderType==="Delivery" && pincode && !/^\\d{6}$/.test(pincode)) return {error:"Please enter a valid 6-digit pincode."};
